"""
Descriptor objects for entities that are part of the root project.
"""
...
from rootbuild.util import fatal, warning
...
class ComponentInfo(object):
    ...
    def get_rootbuild_fragment(self):
        abstract
...
""" % (self.type_name, self.name, self.parent)
    ...
    def get_prefixed_library_name(self):
        ...
        return 'ROOT%s' % basename

    def get_rootconfig_component_name(self):
...
class TargetGroupComponentInfo(ComponentInfo):
    ...
    def parse(subpath, items):
        ...
        kwargs['add_to_library_groups'] = items.get_list(
            'add_to_library_groups')
        kwargs['has_feature'] = items.get_optional_bool('has_feature', False)
    ...
    def __init__(self, subpath, name, parent, required_libraries = [],
        ...
        self.has_feature = bool(has_feature)
...
class BuiltinComponentInfo(ComponentInfo):
    type_name = 'Builtin'
    ...
    def parse(subpath, items):
        kwargs = ComponentInfo.parse_items(items)
        kwargs['required_libraries'] = items.get_list('required_libraries')
        return BuiltinComponentInfo(subpath, **kwargs)
...
def load_from_path(path, subpath):
    # Load the ROOTBuild.txt file as an .ini format file.
