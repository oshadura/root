...
import rootbuild.componentinfo as componentinfo

from rootbuild.util import fatal, note
...
class ROOTProjectInfo(object):
    @staticmethod
    def load_infos_from_path(rootbuild_source_root):
        def recurse(subpath):
            # Load the ROOTBuild file.
            rootbuild_path = os.path.join(rootbuild_source_root + subpath,
                                          'ROOTBuild.txt')
            if not os.path.exists(rootbuild_path):
                fatal("missing ROOTBuild.txt file at: %r" % (rootbuild_path,))

            # Parse the components from it.
            common,info_iter = componentinfo.load_from_path(rootbuild_path,
                                                            subpath)
    ...
    @staticmethod
    def load_from_path(source_root, rootbuild_source_root):
        infos = list(
            ROOTProjectInfo.load_infos_from_path(rootbuild_source_root))

        return ROOTProjectInfo(source_root, infos)
    ...
    def write_components(self, output_path):
        # Organize all the components by the directory their ROOTBuild file
        # should go in.
        info_basedir = {}
        for ci in self.component_infos:
        ...
        for subpath, infos in info_basedir.items():
            ...
            # Format the components into ROOTbuild fragments.
            fragments = []
...
""" % (" ".join(sorted(subdirectories)),)
            ...
            for ci in infos:
                fragment = ci.get_rootbuild_fragment()
                if fragment is None:
                    continue
            ...
            # Create the ROOTBuild fil[e.
            file_path = os.path.join(directory_path, 'ROOTBuild.txt')
            f = open(file_path, "w")

            # Write the header.
            header_fmt = ';===- %s %s-*- Conf -*--===;'
            header_name = '.' + os.path.join(subpath, 'ROOTBuild.txt')
...
""" % header_string)
    ...
    def write_library_table(self, output_path, enabled_optional_components):
        ...
        for c in self.ordered_component_infos:
            ...
            # Compute the root-config "component name". For historical reasons,
            # this is lowercased based on the library name.
            rootconfig_component_name = c.get_rootconfig_component_name()
            ...
            required_rootconfig_component_names = [
                self.component_info_map[dep].get_rootconfig_component_name()
                for dep in c.required_libraries]

            # Insert the entries for library groups we should add to.
            for dep in c.add_to_library_groups:
                entries[dep][2].append(rootconfig_component_name)

            # Add the entry.
            entries[c.name] = (rootconfig_component_name, library_name,
                               required_rootconfig_component_names,
                               is_installed)
...
""")
  ...
  /// The list of libraries required when linking this component.\n')
    ...
    def get_fragment_dependencies(self):
        ...
        # itself. These include all the ROOTBuild files themselves, as well as
        ...
        build_paths = set()
        for ci in self.component_infos:
            p = os.path.join(self.source_root, ci.subpath[1:], 'ROOTBuild.txt')
            if p not in build_paths:
                yield p
                build_paths.add(p)

        # Gather the list of necessary sources by just finding all loaded
        # modules that are inside the ROOT source tree.
    ...
    def write_cmake_fragment(self, output_path, enabled_optional_components):
        ...
        Generate a CMake fragment which includes all of the collated ROOTBuild
        ...
        integrates ROOTBuild, see CMakeLists.txt in the top-level.
        """
        ...
        # Write the header.
        header_fmt = '\
#===-- %s - ROOTBuild Configuration for ROOT %s-*- CMake -*--===#'
...
#===------------------------------------------------------------------------===#
#
# This file contains the ROOTBuild project information in a format easily
# consumed by the CMake based build system.
#
# This file is autogenerated by root-build, do not edit!
#
#===------------------------------------------------------------------------===#
...
""" % header_string)
        ...
        f.write("""
# ROOTBuild CMake fragment dependencies.
#
...
""")
        self.foreach_cmake_library(
            lambda ci:
              f.write("""\
set_property(GLOBAL PROPERTY ROOTBUILD_LIB_DEPS_%s %s)\n""" % (
    ...
    def write_cmake_exports_fragment(self, output_path, enabled_optional_components):
        ...
        Generate a CMake fragment which includes ROOTBuild library
...
# dependencies of libraries imported from ROOT.
""")
        self.foreach_cmake_library(
            lambda ci:
              f.write("""\
...
def add_magic_target_components(parser, project, opts):
    ...
    # least have an explicit representation in the project ROOTBuild files (and
...
def main():
    ...
    group.add_option("", "--source-root", dest="source_root", metavar="PATH",
                      help="Path to the ROOT source (inferred if not given)",
                      action="store", default=None)
    group.add_option("", "--rootbuild-source-root",
                     dest="rootbuild_source_root",
                     help=(
            "If given, an alternate path to search for ROOTBuild.txt files"),
    ...
    group.add_option("", "--write-rootbuild", dest="write_rootbuild",
                      help="Write out the ROOTBuild.txt files to PATH",
                      action="store", default=None, metavar="PATH")
    ...
    # Determine the ROOT source path, if not given.
    source_root = opts.source_root
    if source_root:
        if not os.path.exists(os.path.join(source_root, 'core', 'base', 'inc',
                                           'RConfig.h')):
            parser.error('invalid ROOT source root: %r' % source_root)
    else:
        rootbuild_path = os.path.dirname(__file__)
        root_build_path = os.path.dirname(rootbuild_path)
        utils_path = os.path.dirname(root_build_path)
        source_root = os.path.dirname(utils_path)
        if not os.path.exists(os.path.join(source_root, 'core', 'base', 'inc',
                                           'RConfig.h')):
            parser.error('unable to infer ROOT source root, please specify')

    # Construct the ROOT project information.
    rootbuild_source_root = opts.rootbuild_source_root or source_root
    project_info = ROOTProjectInfo.load_from_path(
        source_root, rootbuild_source_root)
    ...
    # the schema.
    if opts.write_rootbuild:
        project_info.write_components(opts.write_rootbuild)
