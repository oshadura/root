   ...
   if (newcomp == -1) {
      if (useFirstInputCompression || keepCompressionAsIs) {
         ...
         if (firstInput && !firstInput->IsZombie())
            ...
            newcomp = static_cast<int>(ROOT::RCompressionSetting::EDefaults::kUseGeneralPurpose) % 100;
         delete firstInput;
      } else newcomp = static_cast<int>(ROOT::RCompressionSetting::EDefaults::kUseGeneralPurpose)  % 100; // default compression level.
   }
   if (verbosity > 1) {
