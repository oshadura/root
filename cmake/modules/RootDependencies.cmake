...
function(call find_package)
    if (NOT COMMAND ${find_package})
        message(FATAL_ERROR "Unsupported function/macro \"${find_package}\"")
    else()
        set(_helper "${CMAKE_BINARY_DIR}/helpers/macro_helper_${find_package}.cmake")
        if (NOT EXISTS "${_helper}")
            file(WRITE "${_helper}" "${find_package}(\$\{ARGN\})\n")
        endif()
        include("${_helper}")
    endif()
endfunction()
...
endfunction(find_package_cocoa)

#---Check for Zlib ------------------------------------------------------------------
function(find_package_zlib)
...
#---Check for Freetype---------------------------------------------------------------
function(find_package_freetype)
...
endfunction(find_package_freetype)

#---Check for PCRE-------------------------------------------------------------------
function(find_package_pcre)
  ...
  #      CMAKE_ARGS -G ${CMAKE_GENERATOR} -DCMAKE_INSTALL_PREFIX=${CMAKE_INSTALL_PREFIX}
    ...
    set(PCRE_FOUND ON CACHE INTERNAL "")
  endif()
endfunction(find_package_pcre)

#---Check for LZMA-------------------------------------------------------------------
function(find_package_lzma)
...
endfunction(find_package_lzma)

#---Check for XXHASH-----------------------------------------------------------------
function(find_package_xxhash)
...
endfunction(find_package_xxhash)

#---Check for LZ4--------------------------------------------------------------------
function(find_package_lz4)
...
endfunction(find_package_lz4)

#---Check for X11 which is mandatory lib on Unix--------------------------------------
function(find_package_x11)
  if(x11)
...
endfunction(find_package_opengl)

#---Check for GLEW -------------------------------------------------------------------
function(find_package_glew)
  if(NOT builtin_glew)
...
endfunction(find_package_glew)

#---Check for gl2ps ------------------------------------------------------------------
function(find_package_gl2ps)
...
endfunction(find_package_gl2ps)

#---Check for Graphviz installation-------------------------------------------------------
function(find_package_graphviz)
...
endfunction(find_package_graphviz)

#---Check for Bonjour installation-------------------------------------------------------
function(find_package_bonjour)
...
endfunction(find_package_bonjour)


#---Check for krb5 Support-----------------------------------------------------------
function(find_package_krb5)
...
endfunction(find_package_krb5)

#---Check for XML Parser Support-----------------------------------------------------------
function(find_package_libxml2)
...
endfunction(find_package_libxml2)

#---Check for OpenSSL------------------------------------------------------------------
function(find_package_openssl)
...
endfunction(find_package_openssl)

#---Check for Castor-------------------------------------------------------------------
function(find_package_castor)
...
endfunction(find_package_castor)

#---Check for RFIO-------------------------------------------------------------------
function(find_package_rfio)
...
endfunction(find_package_rfio)

#---Check for MySQL-------------------------------------------------------------------
function(find_package_mysql)
...
endfunction(find_package_mysql)

#---Check for Oracle-------------------------------------------------------------------
function(find_package_oracle)
...
endfunction(find_package_oracle)

#---Check for ODBC-------------------------------------------------------------------
function(find_package_odcb)
...
endfunction(find_package_odcb)

#---Check for PostgreSQL-------------------------------------------------------------------
function(find_package_pqsql)
...
endfunction(find_package_pqsql)

#---Check for SQLite-------------------------------------------------------------------
function(find_package_sqlite)
...
endfunction(find_package_sqlite)

#---Check for Pythia6-------------------------------------------------------------------
function(find_package_pythia6)
...
endfunction(find_package_pythia6)

#---Check for Pythia8-------------------------------------------------------------------
function(find_package_pythia8)
...
endfunction(find_package_pythia8)

#---Check for FFTW3-------------------------------------------------------------------
function(find_package_fftw3)
...
endfunction(find_package_fftw3)

#---Check for fitsio-------------------------------------------------------------------
function(find_package_fitsio)
  if(builtin_cfitsio)
...
endfunction(find_package_fitsio)

#---Check Shadow password support----------------------------------------------------
function(find_package_shadowpw)
...
endfunction(find_package_shadowpw)

#---Monalisa support----------------------------------------------------------------
function(find_package_monalisa)
...
endfunction(find_package_monalisa)

#---Check for Xrootd support---------------------------------------------------------
function(find_package_xrootd)
...
endfunction(find_package_xrootd)

#---Alien support----------------------------------------------------------------
function(find_package_alien)
  if(NOT xrootd)
...
endfunction(find_package_alien)

#---Check for Apache Arrow
function(find_package_arrow)
  find_package(Arrow)
  if(NOT ARROW_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_arrow)

#---Check for cling and llvm --------------------------------------------------------
#function(find_package_cling)
  #set(CLING_INCLUDE_DIRS "${CMAKE_SOURCE_DIR}/interpreter/cling/include" CACHE STRING "Cling include directory" FORCE)
  #if(MSVC)
   # set(CLING_CXXFLAGS "-DNOMINMAX -D_XKEYCHECK_H" CACHE STRING "Cling CXXflags" FORCE)
  #else()
   # set(CLING_CXXFLAGS "-fvisibility=hidden -Wno-shadow -fno-strict-aliasing -Wno-unused-parameter -Wwrite-strings -Wno-long-long" CACHE STRING "Cling CXXflags" FORCE)
  #endif()
  #if (CMAKE_COMPILER_IS_GNUCXX)
   # set(CLING_CXXFLAGS "${CLING_CXXFLAGS} -Wno-missing-field-initializers" CACHE STRING "Cling CXXflags" FORCE)
  #endif()
  #---These are the libraries that we link ROOT with CLING---------------------------
  #set(CLING_LIBRARIES clingInterpreter clingMetaProcessor clingUtils)
  #set(CLING_LIBRARIES ${CLING_LIBRARIES} CACHE STRING "Cling Libraries" FORCE)
  #set(CLING_FOUND ON CACHE INTERNAL "")
  ...
  #endif()
#endfunction(find_package_cling)

#---Check for gfal-------------------------------------------------------------------
function(find_package_gfal)
  find_package(GFAL)
  if(NOT GFAL_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_gfal)


#---Check for dCache-------------------------------------------------------------------
function(find_package_dcache)
  find_package(DCAP)
  if(NOT DCAP_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_dcache)

#---Check for Ldap--------------------------------------------------------------------
function(find_package_ldap)
  find_package(Ldap)
  if(NOT LDAP_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_ldap)

#---Check for globus--------------------------------------------------------------------
function(find_package_globus)
  find_package(Globus)
  if(NOT GLOBUS_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_globus)

#---Check for ftgl if needed----------------------------------------------------------
function(find_package_ftgl)
if(NOT builtin_ftgl)
  find_package(FTGL)
  if(NOT FTGL_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_r)

#---Check for hdfs--------------------------------------------------------------------
function(find_package_hdfs)
  find_package(hdfs)
  if(NOT HDFS_FOUND)
    if(fail-on-missing)
...
endfunction(find_package_hdfs)

#---Check for Davix library-----------------------------------------------------------
function(find_package_davix)
...
endfunction(find_package_davix)

#---Check for TCMalloc---------------------------------------------------------------
function(find_package_tcmalloc)
  message(STATUS "Looking for tcmalloc")
  find_package(tcmalloc)
  if(NOT TCMALLOC_FOUND)
    message(STATUS "TCMalloc not found.")
  endif()
endfunction(find_package_tcmalloc)

#---Check for JEMalloc---------------------------------------------------------------
function(find_package_jemalloc)
  if (tcmalloc)
...
endfunction(find_package_jemalloc)

#---Check for TBB---------------------------------------------------------------------
function(find_package_imt)
  if(NOT builtin_tbb)
...
if(builtin_tbb)
  ...
  set(TBB_TARGET TBB)
endfunction(find_package_imt)

#---Check for OCC--------------------------------------------------------------------
function(find_package_geocad)
...
endfunction(find_package_geocad)

#---Check for Vc compatibility-----------------------------------------------------------
function(find_package_vc)
if(vc OR builtin_vc)
...
endfunction(find_package_vc)

#---Check for VecCore--------------------------------------------------------------------
function(find_package_veccore)
if(builtin_veccore)
  unset(VecCore_FOUND)
...
endfunction(find_package_veccore)

#---Check for Vdt--------------------------------------------------------------------
function(find_package_vdt)
...
endfunction(find_package_vdt)

#---Check for VecGeom--------------------------------------------------------------------
function(find_package_vecgeom)
...
endfunction(find_package_vecgeom)
