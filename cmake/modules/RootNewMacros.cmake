...
macro(generate_module_manifest libname dlist srclist hdrlist)
  ...
  file(WRITE "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" "module: \n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " name: " \"${libname}\" "\n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " packageurl: " \"https://github.com/root-project/${pkgname}\"\n)
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " tag: 0.0.0\n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " path: " "${CMAKE_CURRENT_SOURCE_DIR}\n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " publicheaders: " "${hdrlist_pretty_print}\n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " sources: " "${srclist_pretty_print}\n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " targets: " "${libname}\n")
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/${libname}module.yml" " deps: " "${dlist_pretty_print}\n")
...
macro(add_root_subdirectory name)
   # DEBUG: message(STATUS "[modulariz.]  add_root_subdirectory for module " ${name})
  ...
  # We need to extend it to have as a multi-dimensions search tool
   ...
   if(root-modularization)
      foreach(list_name IN LISTS rootpackagemap_requested)
         foreach(value IN LISTS ${list_name})
            if("${value}" STREQUAL "${name}")
               ...
               #DEBUG: message(STATUS "[modulariz.] DEBUG: " ${list_name} " & " ${value})
               #DEBUG: message(STATUS "[modulariz.] We found " ${listname} " package for the ROOT module " ${name})
            else()
    ...
    # DEBUG: message(STATUS "[modulariz.] Check out what we have required from ROOT packages we want to build: " ${root_packages})
    # listname ?= BASE
    ...
    # If we have return_value TRUE -> then we can go ahead and add our directories
      if(return_value)
         ...
         if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${name}/CMakeLists.txt)
            ...
            # DEBUG: message(STATUS "[modulariz.] We dont have " ${name} " module!")
         endif()
      else()
         #DEBUG: message(STATUS "[modulariz.] Working only with packages which doesn't have " ${name} " module.")
      endif()
   else()
      add_subdirectory(${CMAKE_CURRENT_SOURCE_DIR}/${name} ${name})
   endif()
