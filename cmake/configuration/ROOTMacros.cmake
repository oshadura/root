...
function(ROOT_GENERATE_DICTIONARY dictionary)
  CMAKE_PARSE_ARGUMENTS(ARG "STAGE1;MULTIDICT;NOINSTALL" "MODULE" "LINKDEF;OPTIONS;DEPENDENCIES;BUILTINS;EXTERNAL_DEPENDENCIES" ${ARGN})
...
function(ROOT_LINKER_LIBRARY library)
  CMAKE_PARSE_ARGUMENTS(ARG "DLLEXPORT;CMAKENOEXPORT;TEST;NOINSTALL" "TYPE" "LIBRARIES;DEPENDENCIES;BUILTINS;EXTERNAL_DEPENDENCIES"  ${ARGN})
  ROOT_GET_SOURCES(lib_srcs src ${ARG_UNPARSED_ARGUMENTS})
   if(ARG_EXTERNAL_DEPENDENCIES)
     foreach(dep ${ARG_EXTERNAL_DEPENDENCIES})
       message(STATUS ${dep})
       call(find_package_${dep})
       if(dep MATCHES "^root_*")
         string(REPLACE "root_" "" dep ${dep})
         message(STATUS "!!!!!!!!!!!!!!!!" ${dep})
       endif()
       string(TOUPPER "${dep}" EXTERNAL_DEPENDENCY)
       set(EXTERNAL_FOUND "${EXTERNAL_DEPENDENCY}_FOUND")
       set(EXTERNAL_DEPENDENCIES_INCLUDES "${EXTERNAL_DEPENDENCY}_INCLUDE_DIR")
       #message(STATUS ${EXTERNAL_DEPENDENCIES_INCLUDES})
       set(EXTERNAL_DEFINITIONS "${EXTERNAL_DEPENDENCY}_DEFINITIONS}")
...
function(ROOT_OBJECT_LIBRARY library)
  CMAKE_PARSE_ARGUMENTS(ARG "" "" "BUILTINS;EXTERNAL_DEPENDENCIES"  ${ARGN})
   if(ARG_EXTERNAL_DEPENDENCIES)
     call(find_package_${ARG_EXTERNAL_DEPENDENCIES})
     string(TOUPPER "${ARG_EXTERNAL_DEPENDENCIES}" EXTERNAL_DEPENDENCY)
     set(EXTERNAL_FOUND "${EXTERNAL_DEPENDENCY}_FOUND")
...
function(ROOT_EXECUTABLE executable)
  CMAKE_PARSE_ARGUMENTS(ARG "CMAKENOEXPORT;NOINSTALL;TEST" "" "LIBRARIES;BUILTINS;ADDITIONAL_COMPILE_FLAGS;EXTERNAL_DEPENDENCIES"  ${ARGN})
   if(ARG_EXTERNAL_DEPENDENCIES)
     call(find_package_${ARG_EXTERNAL_DEPENDENCIES})
     string(TOUPPER "${ARG_EXTERNAL_DEPENDENCIES}" EXTERNAL_DEPENDENCY)
     set(EXTERNAL_FOUND "${EXTERNAL_DEPENDENCY}_FOUND")
...
function(ROOT_STANDARD_LIBRARY_PACKAGE libname)
   add_root_component(${libname} ${ARGN})
endfunction()
...
elseif(ARG_NO_HEADERS)
  ...
  ROOT_GENERATE_DICTIONARY(G__${libname} ${ARG_HEADERS}
                          ...
                          EXTERNAL_DEPENDENCY ${ARG_EXTERNAL_DEPENDENCIES}
...
macro(generate_module_manifest libname dlist srclist hdrlist)
  ...
  file(APPEND "${CMAKE_CURRENT_SOURCE_DIR}/module.yml" " packageurl: " \"https://github.com/root-project/root/${pkgname}\"\n)
  ...
  if(${listname})
   ...
   file(APPEND "${CMAKE_BINARY_DIR}/${nameofpackagefile}" "  packageurl: " \"https://github.com/root-project/root/${pkgname}\" "\n")
...
macro(add_subdirectory name)
  # Make sure that we have a genertic lower-case name of the module as an input
  ...
  # We need to extend it to have as a multi-dimensions search tool
   ...
   SET(interpreter_dir ${CMAKE_CURRENT_LIST_DIR})
   message(STATUS ${interpreter_dir})
   if(root-lazy-build)
       message(STATUS "[modulariz.] We have " ${PROJECT_NAME} " project ;D")
       # we need to introduce a exception for "interpreter" and "core" directory
       if("${name}" STREQUAL "cling")
         message(STATUS "[modulariz.] We have " ${name} " from Cling module :D")
         _add_subdirectory(${CMAKE_CURRENT_SOURCE_DIR}/${name} ${name})
       elseif("${PROJECT_NAME}" STREQUAL "LLVM")
         message(STATUS "[modulariz.] We have " ${name} " from LLVM module ;D")
         if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${name})
           _add_subdirectory(${CMAKE_CURRENT_SOURCE_DIR}/${name} ${name})
         else()
           add_llvm_subdirectory(LLVM TOOL ${name})
         endif()
      elseif("${PROJECT_NAME}" STREQUAL "Cling")
          message(STATUS "[modulariz.] We have " ${name} " from Cling module ;D")
          if(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${name})
            _add_subdirectory(${CMAKE_CURRENT_SOURCE_DIR}/${name} ${name})
          else()
            add_cling_subdirectory(${name})
          endif()
       else()
...
macro(generate_package_manifest name filename)
  file(WRITE "${CMAKE_BINARY_DIR}/${filename}" "package: \n")
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" " name: " \"${name}\" \n)
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" " tag: 0.0.0" \n)
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" " targets:\n")
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" "  target:\n")
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" "   name: \"${name}\"\n")
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" " products:\n")
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" "  package:\n")
  file(APPEND "${CMAKE_BINARY_DIR}/${filename}" "   name: ${name}\n")
endmacro()
