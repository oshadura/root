...
#######################################################################################
function(call function_package)
    if(NOT COMMAND ${function_package})
        message(FATAL_ERROR "Unsupported function/macro \"${function_package}\"")
    else()
        set(_helper "${CMAKE_BINARY_DIR}/helpers/macro_helper_${function_package}.cmake")
        if(NOT EXISTS "${_helper}")
            file(WRITE "${_helper}" "${function_package}(\$\{ARGN\})\n")
        endif()
        include("${_helper}")
    endif()
endfunction()
...
if(NOT builtin_lzma)
  ...
  foreach(suffix FOUND LIBRARY INCLUDE_DIR)
    unset(LIBLZMA_${suffix} CACHE)
  endforeach()
...
#FIXME: please combine this and next function together
...
function(find_package_builtin_asimage)
...
function(find_package_libxml2)
  ...
  set(${LIBXML2_INCLUDE_DIR} PARENT_SCOPE)
  set(${LIBXML2_DEFINITIONS} PARENT_SCOPE)
  list(APPEND EXTERNAL_INCLUDE_DIRS ${LIBXML2_INCLUDE_DIR})
  list(APPEND EXTERNAL_DEFINITIONS ${LIBXML2_DEFINITIONS})
  set(EXTERNAL_INCLUDE_DIRS ${EXTERNAL_INCLUDE_DIRS} PARENT_SCOPE)
  set(EXTERNAL_DEFINITIONS ${EXTERNAL_DEFINITIONS} PARENT_SCOPE)
# FIXME: fix a return from here to RootFramework
endfunction()
...
#---Check for RFIO-------------------------------------------------------------------
function(find_package_dpm)
  message(STATUS "Looking for RFIO")
  find_package(Castor)
  find_package(DPM)
  if(NOT CASTOR_FOUND AND NOT DPM_FOUND)
    message(STATUS "Castor or DPM not found. Switching off - cant build RFIO package")
  endif()
endfunction()
...
function(find_package_buitin_tbb)
...
function(find_package_tmvacuda)
